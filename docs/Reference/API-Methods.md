...
- All JSON-RPC HTTP examples use the default host and port endpoint `http://127.0.0.1:8545`. If using the [`--http-listen-host`](CLI/CLI-Syntax.md#http-listen-host) or [`--http-listen-port`](CLI/CLI-Syntax.md#http-listen-port) options, update the endpoint.
...
Creates and signs a [private transaction](https://besu.hyperledger.org/en/stable/Concepts/Privacy/Privacy-Overview/) using the [signing key](../Concepts/Overview.md).
...
Besu uses a Transaction Manager to implement privacy. [Tessera](http://docs.tessera.consensys.net) is the Transaction Manager used in this documentation but EthSigner can be used with other Transaction Managers.
